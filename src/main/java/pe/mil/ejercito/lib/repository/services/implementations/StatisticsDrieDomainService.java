...
import pe.mil.ejercito.lib.repository.components.mappers.IStatisticsDrieMapper;
import pe.mil.ejercito.lib.repository.components.validations.IStatisticsDrieValidation;
...
import pe.mil.ejercito.lib.repository.repositories.IEpStatisticsDrieRepository;
...
import pe.mil.ejercito.lib.repository.services.contracts.IStatisticsDrieDomainService;
...
/**
 * StatisticsDrieDomainService
 * <p>
 * StatisticsDrieDomainService class.
 * <p>
...
@Log4j2
@Service
public class StatisticsDrieDomainService extends ReactorServiceBase implements IStatisticsDrieDomainService {

    private final IEpStatisticsDrieRepository repository;
    private final IEpDocumentRegisterRepository documentRegisterRepository;
    private final IStatisticsDrieMapper mapper;

    public StatisticsDrieDomainService(final IEpStatisticsDrieRepository repository,
                                       final IEpDocumentRegisterRepository documentRegisterRepository,
                                       final IStatisticsDrieMapper mapper) {
    ...
    @Override
    public Mono<StatisticsDrieDto> saveEntity(StatisticsDrieDto dto) {
        return doOnSave(dto)
    ...
    private Mono<StatisticsDrieDto> doOnValidateResponse(StatisticsDrieDto dto) {
        return IStatisticsDrieValidation.doOnValidationResponse().apply(dto)
            .flatMap(validation -> {
    ...
    private Mono<StatisticsDrieDto> doOnSave(StatisticsDrieDto dto) {
        return Mono.just(dto)
            .flatMap(request -> {
                final EpStatisticsDrieEntity persistenceEntity = this.mapper.mapperToEntity(request);
                ...
                final EpStatisticsDrieEntity entityResult = this.repository.save(persistenceEntity);
