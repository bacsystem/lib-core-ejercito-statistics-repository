...
import pe.mil.ejercito.lib.repository.components.mappers.IStatisticsOpreMapper;
import pe.mil.ejercito.lib.repository.components.validations.IStatisticsOpreValidation;
...
import pe.mil.ejercito.lib.repository.repositories.IEpStatisticsOpreRepository;
...
import pe.mil.ejercito.lib.repository.services.contracts.IStatisticsOpreDomainService;
...
/**
 * StatisticsOpreDomainService
 * <p>
 * StatisticsOpreDomainService class.
 * <p>
...
@Log4j2
@Service
public class StatisticsOpreDomainService extends ReactorServiceBase implements IStatisticsOpreDomainService {

    private final IEpStatisticsOpreRepository repository;
    private final IEpDocumentRegisterRepository documentRegisterRepository;
    private final IStatisticsOpreMapper mapper;

    public StatisticsOpreDomainService(final IEpStatisticsOpreRepository repository,
                                       final IEpDocumentRegisterRepository documentRegisterRepository,
                                       final IStatisticsOpreMapper mapper) {
    ...
    @Override
    public Mono<StatisticsOpreDto> saveEntity(StatisticsOpreDto dto) {
        return doOnSave(dto)
    ...
    private Mono<StatisticsOpreDto> doOnValidateResponse(StatisticsOpreDto dto) {
        return IStatisticsOpreValidation.doOnValidationResponse().apply(dto)
            .flatMap(validation -> {
    ...
    private Mono<StatisticsOpreDto> doOnSave(StatisticsOpreDto dto) {
        return Mono.just(dto)
            .flatMap(request -> {
                final EpStatisticsOpreEntity persistenceEntity = this.mapper.mapperToEntity(request);
                ...
                persistenceEntity.setStoDocument(documentRegisterEntity.get());
                persistenceEntity.setStCreatedDate(Instant.now());

                final EpStatisticsOpreEntity entityResult = this.repository.save(persistenceEntity);
