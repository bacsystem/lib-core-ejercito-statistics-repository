...
import pe.mil.ejercito.lib.repository.components.mappers.IStatisticsCoedeMapper;
import pe.mil.ejercito.lib.repository.components.validations.IStatisticsCoedeValidation;
...
import pe.mil.ejercito.lib.repository.repositories.IEpStatisticsCoedeRepository;
...
import pe.mil.ejercito.lib.repository.services.contracts.IStatisticsCoedeDomainService;
...
/**
 * StatisticsCoedeDomainService
 * <p>
 * StatisticsCoedeDomainService class.
 * <p>
...
@Log4j2
@Service
public class StatisticsCoedeDomainService extends ReactorServiceBase implements IStatisticsCoedeDomainService {

    private final IEpStatisticsCoedeRepository repository;
    private final IEpDocumentRegisterRepository documentRegisterRepository;
    private final IStatisticsCoedeMapper mapper;

    public StatisticsCoedeDomainService(final IEpStatisticsCoedeRepository repository,
                                        final IEpDocumentRegisterRepository documentRegisterRepository,
                                        final IStatisticsCoedeMapper mapper) {
    ...
    @Override
    public Mono<StatisticsCoedeDto> saveEntity(StatisticsCoedeDto dto) {
        return doOnSave(dto)
    ...
    private Mono<StatisticsCoedeDto> doOnValidateResponse(StatisticsCoedeDto dto) {
        return IStatisticsCoedeValidation.doOnValidationResponse().apply(dto)
            .flatMap(validation -> {
    ...
    private Mono<StatisticsCoedeDto> doOnSave(StatisticsCoedeDto dto) {
        return Mono.just(dto)
            .flatMap(request -> {
                final EpStatisticsCoedeEntity persistenceEntity = this.mapper.mapperToEntity(request);
                ...
                final EpStatisticsCoedeEntity entityResult = this.repository.save(persistenceEntity);
